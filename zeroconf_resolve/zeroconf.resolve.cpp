...
public:
    Service(const std::string &domain,
            ...
            zeroconf_resolve *x)
    : NetService(domain, type, name, interface)
    , mpExternal(x)
    {
        setListener(this);
    }
...
void zeroconf_resolve_poll(zeroconf_resolve *x)
{
    // poll for results
    if(!NOGOOD(x)) {
...
void *zeroconf_resolve_new(t_symbol *s, long argc, t_atom *argv)
{
    ...
    if ((x = (zeroconf_resolve *)object_alloc(zeroconf_resolve_class)))
    {
        ...
        x->clock = clock_new((t_object *)x, (method)zeroconf_resolve_poll);
...
int T_EXPORT main(void)
{
    ...
    CLASS_ATTR_SYM(c, "interface", 0, zeroconf_resolve, interface);
    
    class_register(CLASS_BOX, c); /* CLASS_NOBOX */
    zeroconf_resolve_class = c;
    
    post("| zeroconf.resolve - code by Remy Muller");
    post("| This maintenance release by Nick Rothwell, nick@cassiel.com");
    post("| $Id: zeroconf.resolve.cpp,v cfa525e1c723 2011/02/26 22:01:01 nick $");
